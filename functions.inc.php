...
function weakPwd($pwd) {
	$result;
	if ((strlen(trim($pwd)))< 8){	//password needs to be at least 8 characters
		$result = true;
	}
	else if (!preg_match('/[A-Z]+/', $pwd)){ 	//password needs to contain at least one capital letter
		$result = true;
	}
	else if (!preg_match('/[0-9]+/', $pwd)){	//password needs to contain at least one number
		$result = true;
	}
	else if (!preg_match('/[!@#\$%\^&\*]+/', $pwd)){	//password needs to contain at least one of the following: !, @, #, $, %, ^, &. *
		$result = true;
	}
	else {
		$result = false;
